...
- 네트워크 지연 추정은 인증 없이 동일 JEV API에 POST한 401/403 응답 3회의 중앙값이다. 키·사용자 입력은 전송하지 않고 최대 5분 재사용한다.
- 판단·처리 시간 추정은 JEV 왕복에서 위 기준값을 뺀 값이다. 인증 처리·대기열·연결 상태 차이를 포함하므로 순수 모델 실행 시간은 아니다. 기준값이 정상 요청보다 크거나 만료·측정 실패 시 추정 불가로 표시한다.
...
새로고침은 저장된 카드만 복원하며 자동 재전송하지 않는다. 복원 시 스크롤은 즉시 이동한다. 입력값은 전송 즉시 비우며 Enter 반복/IME 이벤트를 차단한다. 자연어 요청 ID가 동일한 재요청은 단일 서버 프로세스에서 5분 동안 결과를 재사용한다(최대 200건, 서버 재시작 시 초기화).

처리 과정은 인증 실패 기준값으로 판단·처리와 JEV 네트워크 지연을 각각 추정한다. 브라우저↔앱 네트워크는 전체 응답에서 앱 서버 처리 시간을 뺀 별도 추정값이며 직렬화/브라우저 오버헤드를 포함한다. JEV_NETWORK_PROBE=0으로 인증 실패 측정을 끌 수 있다. 기존 저장 메시지에는 새 측정값을 소급 적용하지 않는다.

각 응답 카드 아래에도 총 응답 시간(판단·처리 추정시간), 예상 USD / KRW 비용을 표시한다. 직접 선택은 판단 0ms·호출 없음, 저장된 과거 응답에 측정값이 없으면 — 또는 추정 불가로 표시한다.
